...
if(isset($_POST['submit'])){
   ...
   $select_users = mysqli_query($conn, "SELECT * FROM `users` WHERE email = '$email' AND number = '$number' AND password = '$pass'") or die('query failed');
   ...
   if(mysqli_num_rows($select_users) > 0){
      ...
      if($pass != $cpass){
         ...
         mysqli_query($conn, "INSERT INTO `users`(name, number, email, password) VALUES('$name', '$number', '$email', '$cpass')") or die('query failed');
